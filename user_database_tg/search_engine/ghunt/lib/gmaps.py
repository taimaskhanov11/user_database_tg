...
async def scrape(gaiaID, client, cookies, config, headers, regex_rev_by_id, is_headless, account_info):
    ...
    tmprinter.clear()
    logger.trace(
        f"[+] Average rating : {int(rating_avg) if int(rating_avg) / round(rating_avg, 1) == 1 else round(rating_avg, 1)}/5 stars !"
    )
...
def get_confidence(geolocator, data, gmaps_radius):
    ...
    locations = {
        k: v for k, v in sorted(locations.items(), key=lambda k: len(k[1]["locations"]), reverse=True)
    }  # We sort it
