...
class NotGoogleAccountError(Exception):
    pass
...
async def is_email_google_account(httpx_client: httpx.AsyncClient, auth, cookies, email, hangouts_token):
    ...
    req = await httpx_client.post(host + url, data=body.format(email), headers=headers, cookies=cookies)
...
async def get_account_data(httpx_client, gaiaID, internal_auth, internal_token, config):
    # Bypass method
    req_headers = {
    ...
    req = await httpx_client.get(url, headers=headers)
    data = json.loads(req.text)
    # pprint(data)
    if "error" in data and "Request had invalid authentication credentials" in data["error"]["message"]:
        logger.exception("[-] Cookies/Tokens seems expired, please verify them.")
    elif "error" in data:
    ...
    name = await get_account_name(httpx_client, gaiaID, data, internal_auth, internal_token, config)
...
async def get_account_name(httpx_client, gaiaID, data, internal_auth, internal_token, config):
    try:
    ...
    req = await httpx_client.get(f"https://www.google.com/maps/contrib/{gaiaID}")
