...
async def fetch(email, client, config, account_info):
    ...
    account_info["Google Calendar"] = url_endpoint
    logger.trace("\nGoogle Calendar : " + url_endpoint)
    req = await client.get(url_endpoint + "&hl=en")
    source = req.text
    try:
    ...
    json_calendar_endpoint = assemble_api_req(
        calendarId, singleEvents, maxAttendees, maxResults, sanitizeHtml, timeMin, API_key, email
    )
    req = await client.get(json_calendar_endpoint)
