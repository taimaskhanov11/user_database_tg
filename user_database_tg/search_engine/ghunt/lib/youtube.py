...
def get_channel_data(client, channel_url):
    ...
    channel_details = about_tab["content"]["sectionListRenderer"]["contents"][0]["itemSectionRenderer"]["contents"][0][
    ...
    out["views"] = (
        channel_details["viewCountText"]["simpleText"].split(" ")[0] if "viewCountText" in channel_details else None
    )
    out["joined_date"] = (
        channel_details["joinedDateText"]["runs"][1]["text"] if "joinedDateText" in channel_details else None
    )
...
async def youtube_channel_search(client, query):
    try:
        link = "https://www.youtube.com/results?search_query={}&sp=EgIQAg%253D%253D"
        req = await client.get(link.format(urllib.parse.quote(query)))
        source = req.text
        data = json.loads(
            source.split('window["ytInitialData"] = ')[1].split('window["ytInitialPlayerResponse"]')[0].split(";\n")[0]
        )
        channels = data["contents"]["twoColumnSearchResultsRenderer"]["primaryContents"]["sectionListRenderer"][
            "contents"
...
async def youtube_channel_search_gdocs(client, query, data_path, gdocs_public_doc):
    ...
    for profile_url in channels:
        ...
        retries = 2
        for retry in list(range(retries))[::-1]:
            req = await client.get(profile_url, follow_redirects=True)
            source = req.text
            try:
                data = json.loads(source.split("var ytInitialData = ")[1].split(";</script>")[0])
                avatar_link = data["metadata"]["channelMetadataRenderer"]["avatar"]["thumbnails"][0]["url"].split("=")[
                    0
                ]
...
async def get_channels(client, query, data_path, gdocs_public_doc):
    from_youtube = await youtube_channel_search(client, query)
    from_gdocs = await youtube_channel_search_gdocs(client, query, data_path, gdocs_public_doc)
    to_process = []
    if from_youtube:
...
def get_confidence(data, query, hash):
    ...
    channels = sorted(
        [json.loads(chan) for chan in set([json.dumps(channel) for channel in channels])],
