...
def get_authorization_source(cookies):
    ...
    req = httpx.get("https://docs.google.com/document/u/0/?usp=direct_url", cookies=cookies, headers=config.headers)

    if req.status_code == 200:
        req2 = httpx.get("https://hangouts.google.com", cookies=cookies, headers=config.headers)
...
def get_internal_tokens(driver, cookies, tmprinter):
    ...
    tmprinter.out(f"Waiting for the authorization header, it " "can takes a few minutes...")
...
def get_clientauthconfig_key(cookies):
    ...
    req = httpx.get("https://console.cloud.google.com", cookies=cookies, headers=config.headers)
...
if __name__ == "__main__":
    ...
    cookies = {
        ...
        "LSID": "",
        "__Secure-3PSID": "",
        "CONSENT": config.default_consent_cookie,
        "PREF": config.default_pref_cookie,
    }
