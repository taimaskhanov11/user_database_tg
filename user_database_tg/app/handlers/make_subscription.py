...
async def subscribe(
    ...
    if bill:
        ...
        bill = await p2p.bill(
            bill_id=int(bill_id), amount=amount, lifetime=15, comment=comment
        )
        await Billing.create_bill(
            db_user, bill.bill_id, amount, duration, day_limit
