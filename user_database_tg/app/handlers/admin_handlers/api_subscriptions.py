...
from user_database_tg.db.models import ApiSubscriptionInfo


class CreateSubscriptionStateAPI(StatesGroup):
    title = State()
    # duration = State()
    daily_limit = State()
    price = State()
...
class ChangeSubscriptionStateAPI(StatesGroup):
    choice_field = State()
    edit_field = State()
...
async def view_all_subscriptions(call: types.CallbackQuery):
    ...
    await call.message.answer(
        f"Текущие подписки {len(SUBSCRIPTIONS_INFO_API)}." f" Для изменения нажмите на соответствующую подписку\n",
        reply_markup=admin_menu.get_current_sub_info_api(),
    )
...
async def change_subscription_info_start(call: types.CallbackQuery, state: FSMContext):
    ...
    await ChangeSubscriptionStateAPI.next()
...
async def change_subscription_info_end(message: types.Message, state: FSMContext):
    ...
    await sub_info.save()
    await SUBSCRIPTIONS_INFO_API[sub_info.pk].refresh_from_db()

    await message.answer("Данные подписки успешно изменены", reply_markup=admin_menu.admin_start)
    await ChangeSubscriptionStateAPI.choice_field.set()


async def create_subscription_start(call: types.CallbackQuery, state: FSMContext):
    # await state.finish()
    await CreateSubscriptionStateAPI.first()
    await call.message.delete()
    await call.message.answer(
        f"1. Введите описание API подписки которое будет отображаться на кнопке (не более 255 символов).\n"
    )
...
async def create_subscription_title(message: types.Message, state: FSMContext):
    ...
    await CreateSubscriptionStateAPI.next()
...
async def create_subscription_price(message: types.Message, state: FSMContext):
    try:
        ...
        print(data)
        new_sub_info = await ApiSubscriptionInfo.create(**data)
        SUBSCRIPTIONS_INFO_API[new_sub_info.pk] = new_sub_info
...
def register_admin_api_subscription_settings_handlers(dp: Dispatcher):
    # user_id = [1985947355, 2014301618]
    dp.register_callback_query_handler(view_all_subscriptions, text="view_all_subscriptions_api")
    dp.register_callback_query_handler(view_subscription_info, text_startswith="view_subscription_api_")

    dp.register_callback_query_handler(change_subscription_info_start, state=ChangeSubscriptionStateAPI.choice_field)
    dp.register_message_handler(change_subscription_info_end, state=ChangeSubscriptionStateAPI.edit_field)

    dp.register_callback_query_handler(create_subscription_start, text="create_subscription_api")
    dp.register_message_handler(create_subscription_title, state=CreateSubscriptionStateAPI.title)
    # dp.register_message_handler(create_subscription_duration, state=CreateSubscriptionStateAPI.duration)
    dp.register_message_handler(create_subscription_daily_limit, state=CreateSubscriptionStateAPI.daily_limit)
    dp.register_message_handler(create_subscription_price, state=CreateSubscriptionStateAPI.price)
